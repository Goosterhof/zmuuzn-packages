...
describe('UserMenu', () => {
    it('should render user name when user is provided', () => {
        const wrapper = mount(UserMenu, {props: {user: {name: 'Goos'}, currentExperiment: 'gatekeeper'}});
    ...
    it('should show correct exit label for each experiment', async () => {
        ...
        for (const exp of cases) {
            const wrapper = mount(UserMenu, {props: {user: {name: 'Goos'}, currentExperiment: exp.id}});
            await wrapper.find('button').trigger('click');
