...
const emit = defineEmits<{depart: [url: string]}>();
...
const computeOffset = (active: boolean, hovered: boolean): string => {
    if (active) return '3px 3px 0';
    if (hovered) return '1px 1px 0';
    return '2px 2px 0';
};

const computedShadow = computed(() => {
    const offset = computeOffset(isActive.value, isHovered.value);
    ...
    if (!neverVisited.value) {
        layers.push(`0 0 ${String(ghostSpread.value)}px rgba(${accentRgb.value}, ${String(ghostOpacity.value)})`);
    }
...
<template>
    <a
        ...
        :style="{...computedBorder, boxShadow: computedShadow, borderRadius: computedRadius}"
    ...
    >
        ...
        <span v-if="isActive" class="map-text-badge block mt-4px" :style="{color: experiment.accentColor}">
            &#9679; YOU ARE HERE
        </span>
        <span
            v-else
...
.room-tile:hover:not(.room-tile--departing):not(.room-tile--sibling-fade):not([aria-current='page']) {
