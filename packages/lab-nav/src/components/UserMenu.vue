...
const exitLabel = computed(() => experiments.find((e) => e.id === currentExperiment)?.exitLabel ?? 'Logout');
...
<template>
    <div ref="menuRef" relative>
        ...
        >
            {{ user.name }}
            <span text-xs transition-transform duration-200 :class="menuOpen ? 'rotate-180' : ''">&#9660;</span>
        </button>
