...
const {currentExperiment} = defineProps<{currentExperiment: ExperimentId}>();
        ...
        <ul flex items-center gap-1 list-none m-0 p-0>
            <li v-for="(exp, index) in experiments" :key="exp.id">
                <span
                    ...
                    gap-2
                    border-b-2
                    :style="{borderBottomColor: accentColor, boxShadow: `0 2px 8px ${accentColor}66`}"
                >
                    <i
                        inline-block
                        w-1.5
                ...
                >
                    <i
                        ...
                        :style="{backgroundColor: exp.accentColor, opacity: 0.4, transition: 'opacity 200ms ease'}"
                    />
                    {{ exp.label }}
                </a>
            </li>
